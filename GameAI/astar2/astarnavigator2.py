...
def clearShot(p1, p2, worldLines, worldPoints, agent):
    ...
    badPoints = worldPoints;
    agentSize = agent.getMaxRadius();
    for point in badPoints:
    	if minimumDistance((p1, p2), point) <= agentSize:
    		return False
...
def getOnPathNetwork(location, pathnodes, worldLines, agent):
	...
	for lines in worldLines:
		worldPoints.append(lines[0])
		worldPoints.append(lines[1])
	for node in pathnodes:
		if clearShot(location, node, worldLines, worldPoints, agent):
			currDist = distance(location, node)
			heapq.heappush(queue, (currDist, node))
	node = queue[0][1]

	### YOUR CODE GOES ABOVE HERE ###u
	return node
...
def astar(init, goal, network):
	...
	queue.append((distance(init, goal), init, opened))
	...
	while len(queue) != 0:
		...
		if curr not in closed:
			closed.append(curr)
		neighbors = getNeighbors(curr, network)
		for neighbor in neighbors:
			if neighbor not in closed:
				tentativeScore = g[curr] + distance(curr, neighbor)
				if neighbor not in g or g[neighbor] > tentativeScore:
					cameFrom[neighbor] = curr
					g[neighbor] = tentativeScore
					fScore = g[neighbor] + distance(neighbor, goal)
					f[neighbor] = fScore
					heapq.heappush(queue, (f[neighbor], neighbor, opened + [neighbor]))
...
def getNeighbors(curr, network):
    neighbors = [node[1] for node in network if node[0] == curr] + \
                 [node[0] for node in network if node[1] == curr]
...
def myUpdate(nav, delta):
	...
	agentLocation = nav.agent.getLocation()
	target = nav.agent.moveTarget
	if not rayTraceWorld(agentLocation, target, gates):
		nav.world.getAgent().stopMoving()
		nav.setPath(None)
		return None
	for gate in gates:
		if nav.getDestination() == None or minimumDistance(gate, agentLocation) < nav.agent.getMaxRadius():
			nav.world.getAgent().stopMoving()
			nav.setPath(None)
			break
	### YOUR CODE GOES ABOVE HERE ###
...
def shortcutPath(source, dest, path, world, agent):
	...
	if clearShot(source, dest, badLines, badPoints, agent):
		path = []
		return path
	for node in path:
...
def mySmooth(nav):
	...
	badPoints = nav.world.getPoints()
	badLines = nav.world.getLines()
	if agentLocation and destination:
		if clearShot(agentLocation, destination, badLines, badPoints, nav.agent) == True:
			nav.agent.moveToTarget(destination)
			nav.path = []
			return True
	### YOUR CODE GOES ABOVE HERE ###
	return False
