...
def treeSpec(agent):
	...
	# spec = [(Selector, "root"), (Retreat, 0.5, "retreat"), [(AreaEffectDaemonCheck, 0.5), [(Selector, "chase hero then minion"), [Sequence, ChaseHero, KillHero], [Sequence, ChaseMinion, KillMinion]]]]
	spec = [(Selector, "root"), [(Selector, "retreat or fight"), [(MinionInRange, 0.5, "go kill minion then go home"), (KillMinion, "kill minion and go back")], (Retreat, 0.5, "retreat")], [(AreaEffectDaemonCheck, 0.5), [(Selector, "chase hero then minion"), [Sequence, ChaseHero, KillHero], [Sequence, ChaseMinion, KillMinion]]]]
...
def myBuildTree(agent):
	...
	# retreat = makeNode(Retreat, agent, 0.4, "retreat if hp < 0.5")
	# fight = makeNode(HitpointDaemon, agent, 0.6, "fight if hp >= 0.5")
	# fight_minion_or_hero = makeNode(Selector, agent, "fight minion or hero")
	# fight_hero = makeNode(BuffDaemon, agent, 1, "fight hero if (my) level is at least 1 more")
	# hero_sequence = makeNode(Sequence, agent, "sequence for fighting hero")
	# chase_hero = makeNode(ChaseHero, agent, "chase the hero")
	# kill_hero = makeNode(KillHero, agent, "kill the hero")
	# minion_seq = makeNode(Sequence, agent, "sequence for fighting minion")
	# chase_minion = makeNode(ChaseMinion, agent, "chase the minion")
	# kill_minion = makeNode(KillMinion, agent, "kill the minion")
	# root.addChild(retreat)
	# root.addChild(fight)
	# fight.addChild(fight_minion_or_hero)
	# fight_minion_or_hero.addChild(fight_hero)
	# fight_minion_or_hero.addChild(minion_seq)
	# fight_hero.addChild(hero_sequence)
	# hero_sequence.addChild(chase_hero)
	# hero_sequence.addChild(kill_hero)
	# minion_seq.addChild(chase_minion)
	# minion_seq.addChild(kill_minion)
	### YOUR CODE GOES ABOVE HERE ###
	return root
...
class Taunt(BTNode):
	...
	def execute(self, delta = 0):
		...
		if self.target is not None:
			print("Hey", self.target, "he's mine!")
		return ret
...
class Retreat(BTNode):
	...
	def execute(self, delta = 0):
		...
		if self.agent.getHitpoints() > self.agent.getMaxHitpoints() * self.percentage:
			...
			# 	if enemy in self.agent.getVisible() and distance(self.agent.getLocation(), enemy.getLocation()) < BULLETRANGE:
	...
	def dodgeBullet(self):
		...
		for bullet in bullets:
			if bullet.getOwner().getTeam()!= self.agent.getTeam():
				...
				vecRange = (vector[0]*self.agent.getMaxRadius(), vector[1]*self.agent.getMaxRadius())
				if minimumDistance([bullet.position, vecRange], self.agent.getLocation()) < self.agent.getMaxRadius():
					self.agent.dodge((angle + math.pi / 2))
				else:
					self.agent.dodge((angle - math.pi / 2))
##################
### ChaseMinion
...
class MinionInRange(BTNode):
	
	### percentage: range of minion has to be 
...
class AreaEffectDaemonCheck(BTNode):
	...
	def execute(self, delta = 0):
		ret = BTNode.execute(self, delta)
		enemies = self.agent.world.getEnemyNPCs(self.agent.getTeam())
		for i in self.agent.getVisible():
			if i in enemies:
				if distance(self.agent.getLocation(), i.getLocation()) < AREAEFFECTRANGE * self.agent.getMaxRadius():
