...
public class MyLevelGenerator{
	...
	private MyDNA generateRandomIndividual ()
	{
		...
		String randomGene = "";
		String original = "tanluvsmikey";
		for (int i = 0; i < 200; i++) {
			randomGene += original.charAt(r.nextInt(original.length()));
		}
		//now add random gene to individual's chromosome set
		individual.setChromosome(randomGene);
	...
	private boolean terminate (ArrayList<MyDNA> population, int count)
	{
		...
		// YOUR CODE GOES BELOW HERE
		//I am terminating after 1000 iterations or the fitness is above 80%
		if (count > 1000) {
			decision = true; //after 1000 iterations
		} else if (!population.isEmpty()){
			// else if a random individual in the population has a fitness of over 80 also terminate:
			for (int i = 0; i < population.size(); i++) {
				MyDNA individual = population.get(i);
				if (individual.getFitness() > 0.80) {
					decision = true;
				}
			}
	...
	private ArrayList<MyDNA> selectIndividualsForMutation (ArrayList<MyDNA> population)
	{
		...
		// YOUR CODE GOES BELOW HERE
		int max = r.nextInt(population.size()) + 1;
		for (int i = 0; i < max; i++) {
	...
	private ArrayList<MyDNA> globalCompetition (ArrayList<MyDNA> oldPopulation, ArrayList<MyDNA> newPopulation)
	{
		...
		if (allPopulation.size() != this.getPopulationSize()) {
			for (int i = 0; i < this.getPopulationSize(); i++) {
				MyDNA bestInd = getBestIndividual(allPopulation);
				finalPopulation.add(bestInd);
			}
		} else {
			finalPopulation = allPopulation;
		}
...
//
//		String randomGene = "";
//		String original = dna.getChromosome();
//		Char rp = original.charAt(r.nextInt(original.length()));
//		for (int i = 0; i < 200; i++) {
//			Char rg = original.charAt(r.nextInt(200));
//			if (rg != rp) {
//				randomGene += rg;
//			}
//			rp = rg;
